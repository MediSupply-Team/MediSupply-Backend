...
# Política adicional para leer Secrets Manager (Mapbox Token)
data "aws_iam_policy_document" "secrets_policy" {
  statement {
    actions = [
      "secretsmanager:GetSecretValue"
    ]
    resources = [var.mapbox_token_secret_arn]
  }
}
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "OSRM_URL", value = var.osrm_url },
        { name = "RUTA_SERVICE_URL", value = var.ruta_service_url }
      ]
...
# ALB LISTENER RULE
# ============================================================
resource "aws_lb_listener_rule" "optimizer_path" {
  listener_arn = var.shared_http_listener_arn
  priority     = 31  # Diferente de rutas (30)
